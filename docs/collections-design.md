...
**未做**（也不打算做）：按 reason 拆分队列、写前人工确认。

**后加**：`route --apply --prune` 让成员关系收敛——删除受管命名空间内、标签已不再指向的成员，
受管范围由 `routing.is_managed_path()` 判定；你自己建的目录永不被碰。
...
| 陈旧成员清理 | 不做默认清理；`--prune` 显式开启，且只作用于受管命名空间 |
